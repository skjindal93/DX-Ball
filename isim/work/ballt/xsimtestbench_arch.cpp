...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        HSim__s6 * topvh = 0;
        extern HSim__s6 * createWork_ballt_testbench_arch(const char*);
        topvh = createWork_ballt_testbench_arch("ballt");
...
main(int argc, char **argv) {
  ...
  try {
    ...
    WorkDivide=new Work_divide("divide");
    IeeeStd_logic_textio=new Ieee_std_logic_textio("STD_LOGIC_TEXTIO");
    WorkArray_b=new Work_array_b("array_b");
    HSimConfigDecl *cfg;
 _top_i = new _top();
